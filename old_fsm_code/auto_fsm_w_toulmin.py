...
async def main():
    ...
    parser.add_argument("--use_nl", type=bool, default=False)
    parser.add_argument("--save_fn", type=str, default='results/fsm_EXP_0317_tou_a_c')
    ...
    sampled_df = df_to_argue.sample(n=2, random_state=2)
            ...
            # if no, enter debate
                ...
                for i in range(0, rounds):
                    ...
                    if False:
                        ...
                        thought = 0
                        if args.use_toulmin:
                            toul_res = await generate_res("check", model_agent, example_sentence, 
                                                               chat_history, toulmin, None, None, None, PROMPT_CHECK_TOULMIN_AGENT, 0)
                            tou = load_json(toul_res.choices[0].message.content)
                            while toul_res == False:
                                toul_res = await generate_res("check", model_agent, example_sentence,
                                                               chat_history, toulmin, None, None, None, PROMPT_CHECK_TOULMIN_AGENT, 0)
                                tou = load_json(toul_res.choices[0].message.content)
                            print(tou)
                            if toul_old != tou["part"]:
                                toul_old = tou["part"]
                                new_utter = "We are now talking about the "+toul_old+ " of the sentence, since " + tou["reason"]
                                # change_tou = True
                            else:
                                new_utter = ""
                    ...
                    if i == 0 and args.use_nl:
                        ...
                        # else:
                        teacher_res = await generate_res("teacher", model_teacher, example_sentence, None, None, None, conv_teacher, conv_student, TEACHER_ACT_1 + STRAT_FOR_STATES_TOU[next_state] + TEACHER_ACT_2_TOU, 1)
                        
                        curr_state = next_state
                        FSM_STATES.append(curr_state)

                        check_following_res = await generate_res("eval_s", model_student, example_sentence, teacher_res.choices[0].message.content, STRAT_FOR_STATES_TOU[next_state], None, None, None, CHECK_FOLLOW_FSM_AGENT, 0)
                        ...
                        if ("no" in cs['1'].lower() or "no" in cs['2'].lower()) and next_state in ['1', '4', '2']:
                            ...
                            teacher_res = await generate_res("agents", model_teacher, example_sentence, teacher_res.choices[0].message.content, None, None, None, None, TEACHER_ACT_1 + STRAT_FOR_STATES_TOU[next_state] + pt, 1)
                            check_following_res = await generate_res("eval_s", model_student, example_sentence, teacher_res.choices[0].message.content, STRAT_FOR_STATES_TOU[next_state], None, None, None, CHECK_FOLLOW_FSM_AGENT, 0)
                            cs = load_json(check_following_res.choices[0].message.content)
                            while cs == False:
                                check_following_res = await generate_res("eval_s", model_student, example_sentence, teacher_res.choices[0].message.content, STRAT_FOR_STATES_TOU[next_state], None, None, None, CHECK_FOLLOW_FSM_AGENT, 0)
                    ...
                    if args.use_toulmin:
                        utterance_teacher = new_utter + utterance_teacher
                    chat_history += "teacher: " + utterance_teacher + "\n"
